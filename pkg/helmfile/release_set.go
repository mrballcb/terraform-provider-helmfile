...
func getDiffFile(ctx *sdk.Context, fs *ReleaseSet) (string, error) {
	...
	if helmfileVersion != nil && gte126.Check(helmfileVersion) {
		...
		// Normalize temporary file paths for consistent naming between plan and apply, to prevent false positives in diff_output due to regenerated temporary paths.
		determinisiticOutput = normalizeTemporaryPaths(determinisiticOutput)
	}
...
func DiffReleaseSet(ctx *sdk.Context, fs *ReleaseSet, d ResourceReadWrite, opts ...DiffOption) (string, error) {
	...
	if err != nil {
		...
		if state.Output != "" {
			...
			// Normalize temporary file paths to ensure consistent output between plan and apply
			diff = normalizeTemporaryPaths(diff)
...
// normalizeTemporaryPaths replaces temporary file paths and directory references
// with normalized placeholders to ensure consistent diff_output between plan and apply.
// This prevents Terraform from detecting false changes due to regenerated temporary paths.
func normalizeTemporaryPaths(s string) string {
	// Regex patterns for temporary paths that may appear in helmfile diff output
	patterns := []struct {
		pattern     *regexp.Regexp
		replacement string
	}{
		{
			// Match absolute paths containing .terraform/helmfile/temp-<hash>/
			// Example: /Users/user/project/.terraform/helmfile/temp-abc123/chart.yaml
			pattern:     regexp.MustCompile(`[^\s]*\.terraform/helmfile/temp-[a-f0-9]+(/[^\s]*)?`),
			replacement: "<temp-dir>$1",
		},
		{
			// Match helmfile-<hash>.yaml temporary files
			// Example: helmfile-abc123def456.yaml
			pattern:     regexp.MustCompile(`helmfile-[a-f0-9]+\.yaml`),
			replacement: "<helmfile-temp>.yaml",
		},
		{
			// Match temp.values-<hash>.yaml temporary values files
			// Example: temp.values-abc123.yaml
			pattern:     regexp.MustCompile(`temp\.values-[a-f0-9]+\.yaml`),
			replacement: "<values-temp>.yaml",
		},
		{
			// Match any /tmp/ or temp directory paths that might be generated by helm/helmfile
			// Example: /tmp/helm-template-workdir-123456/
			pattern:     regexp.MustCompile(`/tmp/[a-zA-Z0-9_-]+/[^\s]*`),
			replacement: "<tmp-dir>",
		},
	}
